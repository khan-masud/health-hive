...
imageInput.addEventListener('change', (event) => {
  ...
  if (file) {
    dropzone.innerHTML = `<p>${file.name}</p>`; // Display the file name after selecting the image
    displayImagePreview(file);
  }
});
...
function openCamera() {
  ...
  const isMobile = /Mobi|Android/i.test(navigator.userAgent); // Simple check for mobile devices
...
captureButton.addEventListener("click", () => {
  ...
  const file = new File([imageBlob], "medical-report.jpg", { type: "image/jpeg" });
  imageInput.files = createFileList(file);
  dropzone.innerHTML = `<p>ইমেজঃ ${file.name}</p>`;
  displayImagePreview(file); // Show preview of captured image
  closeCameraPopup();
});
...
uploadForm.addEventListener("submit", async (e) => {
  ...
  if (!imageFile) {
    resultDiv.innerHTML = `<p style="color: red;">দুঃখিত! একটি মেডিকেল রিপোর্টের ইমেজ দিন।</p>`;
    return;
  }
  ...
  try {
    const response = await fetch(getApiUrl(API_CONFIG.ENDPOINTS.MEDICAL_REPORT), {
      method: "POST",
      body: formData,
    });
